...
async def deploy_full_stack():
    ...
    pi_ssh_key = input("SSH Key (default: ~/.ssh/pi_github_actions): ").strip() or "~/.ssh/pi_github_actions"
    ...
    with RaspberryPiManager(pi_ip, pi_user, ssh_key_path=pi_ssh_key) as pi:
        ...
        for service_name in services:
            try:
                ...
                print(f"   {status_icon} {service_name}: {'active' if status.active else 'inactive'}")
            except Exception:
