    ...
    async def list_keys(self) -> List[TailscaleKey]:
        ...
        for key_data in keys_response.keys:
            keys.append(TailscaleKey(
                ...
                reusable=key_data.capabilities.get("devices", {}).get("create", {}).get("reusable", False),
                ephemeral=key_data.capabilities.get("devices", {}).get("create", {}).get("ephemeral", False),
                preauthorized=key_data.capabilities.get("devices", {}).get("create", {}).get("preauthorized", False),
